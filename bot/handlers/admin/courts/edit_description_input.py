...
class AdminEditCourtDescriptionInput(TextInputHandler):
    ...
    async def _process(self) -> None:
        ...
        await self._update.message.reply_text(
            self._messages.admin_court_updated(name=court.name),
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton(self._messages.btn_edit_another, callback_data='admin_edit_court')],
    ...
    async def _on_error(self, error: Exception) -> None:
        ...
        assert self._update.message is not None
        await self._update.message.reply_text(
            self._messages.admin_court_update_error,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(self._messages.btn_back_to_courts, callback_data='admin_courts')]],
            ),
        )
