...
from bot.handlers.callback_args import AdminDeleteTrainerArg
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f'👨‍🏫 {trainer.user.name}',
                        callback_data=AdminDeleteTrainerArg(id=trainer.id).to_callback_data(),
                    ),
                ],
            )
        keyboard.append([InlineKeyboardButton(self._messages.btn_back, callback_data='admin_trainers')])
        await self._update.callback_query.edit_message_text(
            self._messages.admin_trainer_select_to_delete,
