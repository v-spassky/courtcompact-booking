...
async def _handle_admin_delete_student_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for student in students:
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f'admin_delete_student_{student_id_short}')])

    keyboard.append([InlineKeyboardButton(msgs.btn_back, callback_data='admin_students')])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.callback_query.edit_message_text(msgs.admin_student_select_to_delete, reply_markup=reply_markup)
