...
async def _handle_admin_edit_location_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for location in locations:
        ...
        keyboard.append(
            [InlineKeyboardButton(f'📍 {location.name}', callback_data=f'admin_edit_location_{location_id_short}')]
        )
    keyboard.append([InlineKeyboardButton(msgs.btn_back, callback_data='admin_locations')])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await update.callback_query.edit_message_text(msgs.admin_location_select_to_edit, reply_markup=reply_markup)
